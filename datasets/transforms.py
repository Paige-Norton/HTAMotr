...
#         print((oh, ow))
    ...
    if "proposal_boxes" in target:
        ...
        target["proposal_boxes"] = scaled_boxes
...
#             print(targets_i["texts_ignored"])
            ...
            img_i ,targets_i = img_i,targets_i
            ret_imgs.append(img_i)
            ret_targets.append(targets_i)
        return ret_imgs, ret_targets
...
class MotRandomResize(RandomResize):
    def __call__(self, imgs, targets):
        size = random.choice(self.sizes) ##调整大小
        # size = 1200 ##调整大小
        ret_imgs = []
        ret_targets = []
        for img_i, targets_i in zip(imgs, targets):
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if "boxes" in target:
            ...
            target["boxes"] = boxes
        # if "proposal_boxes" in target:
        #     proposal_boxes = target["proposal_boxes"]
        #     # boxes = box_xyxy_to_cxcywh(boxes)
        #     proposal_boxes = proposal_boxes / torch.tensor([w, h, w, h,w, h, w, h], dtype=torch.float32)
        #     target["proposal_boxes"] = proposal_boxes
        return image, target
